...
from impute import SampleBagger
...
## Test Imputer on small training data
...
def encodeCols(X,columns_to_encode):
    """
...
train_encoded = encodeCols(train,nonnumeric_cols)


null_vals_pct = train_encoded.isnull().sum() / float(len(train_encoded))
print("Null values after Before and imputation")
...
fill_na = SampleBagger(impute_missing_vals=True,
                       ...
                       minibatch=True,
                       minibatch_size=.1)
train_dense = DataFrame(fill_na.genSample(X=train_encoded))
print(train_dense.head())


train_dense.to_csv("data/train_users_dense_TEST.csv")
null_vals_pct = train_dense.isnull().sum() / float(len(train_dense))
print("Null values after Before and imputation")
print(null_vals_pct)

# Expand categorical variables to one hot encoding